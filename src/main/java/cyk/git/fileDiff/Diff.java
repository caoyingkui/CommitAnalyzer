package cyk.git.fileDiff;


import cyk.git.description.Description;
import cyk.git.fileDiff.field.ChangedField;
import cyk.git.fileDiff.field.DelField;
import cyk.git.fileDiff.field.FieldDiff;
import cyk.git.fileDiff.field.NewField;
import cyk.git.fileDiff.file.*;
import cyk.git.fileDiff.method.ChangedMethod;
import cyk.git.fileDiff.method.DelMethod;
import cyk.git.fileDiff.method.MethodDiff;
import cyk.git.fileDiff.method.NewMethod;
import cyk.git.fileDiff.modify.section.ChangedSection;
import cyk.git.fileDiff.rationale.Explainable;
import cyk.git.fileDiff.type.FileType;
import cyk.git.git.ClassParser;
...
public class Diff implements Explainable, Serializable {
    ...
    public FileDiff getFileObject(GitAnalyzer git, ObjectId commit, String fileName) {
        ...
        if (!formerContent.equals("") || oldPath != null && !oldPath.equals("/dev/null")) {
            ...
            if (fileName.compareTo("java/compiler/openapi/src/com/intellij/compiler/ant/taskdefs/Javac.java") == 0) {
                int a = 2;
            }
    ...
    public void parse(GitAnalyzer git, ObjectId commit) {
        ...
        System.out.println(commit.toString() + " " + files.size() + " ");
